    ...
    if (!order) {
      try {
        ...
        const sessionData = {
          ...
          subtotal_amount: stripeSession.amount_subtotal ? stripeSession.amount_subtotal / 100 : 0,
          shipping_amount: stripeSession.total_details?.amount_shipping ? stripeSession.total_details.amount_shipping / 100 : 0,
          shipping_address: stripeSession.shipping_details?.address || null,
          shipping_name: stripeSession.shipping_details?.name || stripeSession.customer_details?.name || null,
